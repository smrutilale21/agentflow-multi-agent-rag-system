...
def load_knowledge_base() -> list[dict]:
    ...
    for pdf_file in pdf_files:
        ...
        documents.append({"source": pdf_file, "content": text})
...
def create_documents(knowledge_base: list[dict]) -> list[Document]:
    ...
    for knowledge in knowledge_base:
        ...
        for index, chunk in enumerate(chunks):
            ...
            documents.append(
                Document(
                    page_content=chunk, metadata={"source": source, "chunk_id": index}
                )
            )
...
def get_vector_store() -> Chroma:
    ...
    if existing_count > 0:
        ...
        logger.info(
            f"Loaded existing Chroma collection with " f"{existing_count} documents"
        )
